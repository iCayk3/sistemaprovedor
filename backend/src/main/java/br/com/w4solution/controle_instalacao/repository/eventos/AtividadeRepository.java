...
public interface AtividadeRepository extends JpaRepository<Atividade, Long> {
    ...
    @Query("SELECT a.evento, COUNT(a) " +
            ...
            "GROUP BY a.evento")
    List<Object[]> encontrarAtividadesPorUsuario(String usuario, Integer mes, Integer ano, Integer dia);
    ...
    Integer encontrarAtividadesMensal(String evento, int monthValue, int year);

    @Query("SELECT a FROM Atividade a WHERE EXTRACT(MONTH FROM a.data) = :mes AND EXTRACT(YEAR FROM a.data) = :ano ORDER BY a.id DESC")
    List<Atividade> listarAtividadesDoMes(Integer ano, Integer mes);
}
